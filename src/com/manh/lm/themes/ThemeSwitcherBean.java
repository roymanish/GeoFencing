...
public class ThemeSwitcherBean {
	...
	private final static String themeCookieName = "_ThemeRoller_Theme___123";
	...
	public ThemeSwitcherBean() {
		...
		// try fetching the themes from cookies
		try {

			Map<String, Object> cookieMap = FacesContext.getCurrentInstance()
					.getExternalContext().getRequestCookieMap();
			if (cookieMap.containsKey(themeCookieName)) {
				// validate that this theme name is there in theme map
				Cookie cookie = (Cookie) cookieMap.get(themeCookieName);
				if (themes.containsValue(cookie.getValue())) {
	...
	public void saveTheme(AjaxBehaviorEvent ajaxBehaviorEvent) {

		try {

			HttpServletResponse response = (HttpServletResponse) FacesContext
					.getCurrentInstance().getExternalContext().getResponse();
			Cookie cookie = new Cookie(themeCookieName, theme);
